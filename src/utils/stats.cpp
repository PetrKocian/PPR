...
void Stats::push_v(__m256d vec_x)
{
	...
	/*m4_v = _mm256_add_pd(m4_v,
		_mm256_add_pd(_mm256_mul_pd(_mm256_mul_pd(term1_v,delta_n2_v),
			_mm256_sub_pd(_mm256_mul_pd(n_v, n_v),
		_mm256_fmadd_pd({ 3.0,3.0,3.0,3.0 },
			n_v, { 3.0,3.0,3.0,3.0 }))),
		_mm256_sub_pd(_mm256_mul_pd(_mm256_mul_pd(delta_n2_v,
			{ 6.0,6.0,6.0,6.0 }),m2_v),
			_mm256_mul_pd(_mm256_mul_pd({ 4.0,4.0,4.0,4.0 }, delta_n_v),
				m3_v))));*/
...
double Stats::variance_v()
{

}

double Stats::skewness_v()
{
...
double Stats::kurtosis_v()
{
	...
	/*
	double delta_f = m1_p[1] - m1_p[0];
	double delta_n_f = delta_f / (n_p[0] + n_p[1]);
	double delta_n2_f = delta_n_f * delta_n_f;
	double delta_n3_f = delta_n_f * delta_n2_f;
	double count_sq = n_p[0] * n_p[0];
	double rn_sq = n_p[1] * n_p[1];

	double m4_p1 = m4_p[0] + m4_p[1] + delta_f * delta_n3_f * n_p[0] * n_p[1] *
		(count_sq - n_p[0] * n_p[1] + rn_sq) + 6 * delta_n2_f * (count_sq * m2_p[1] + rn_sq * m2_p[0]) +
		4 * delta_n_f * (n_p[0] * m3_p[1] - n_p[1] * m3_p[0]);

	double m2_p1 = m2_p[0] + m2_p[1] + delta_f * delta_n_f * n_p[0] * n_p[1];

	double m3_p1 = m3_p[0] + m3_p[1] + delta_f * delta_n_f * n_p[0] * n_p[1] * (n_p[0] - n_p[1]) +
		3 * delta_n_f * (n_p[0] * m2_p[1] - n_p[1] * m2_p[0]);
	*/
	Stats_partial a = combine_m4(n_p[0], n_p[1], m4_p[0], m4_p[1], m3_p[0], m3_p[1], m2_p[0], m2_p[1], m1_p[0], m1_p[1]);
	Stats_partial b = combine_m4(n_p[2], n_p[3], m4_p[2], m4_p[3], m3_p[2], m3_p[3], m2_p[2], m2_p[3], m1_p[2], m1_p[3]);
...
double Stats::kurtosis_complete()
{
	...
	Stats_partial result_final =combine_m4(result.n, n, result.m4, m4, result.m3, m3, result.m2, m2, result.m1, m1);

	return (result_final.m4 * result_final.n) / (result_final.m2 * result_final.m2) - 3;
}

Stats_partial Stats::combine_m4(double count_a, double count_b, double m4_a, double m4_b, double m3_a, double m3_b, double m2_a, double m2_b, double m1_a, double m1_b)
{
	Stats_partial result;

	double delta_f = m1_b- m1_a;
	double count_combined = count_a + count_b;
	double delta_n_f = delta_f / (count_combined);
	double delta_n2_f = delta_n_f * delta_n_f;
	double delta_n3_f = delta_n_f * delta_n2_f;
